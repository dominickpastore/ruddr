...
def notifier_factory():
    """Fixture creating a factory for mock :class:`~ruddr.Notifier`"""
    class NotifierFactory:
        ...
        def __call__(self, **kwargs):
            self._count += 1
            config = kwargs
            return mocks.MockNotifier(f'mock_notifier_{self._count}', config)
...
def test_ipv4_ready_false(notifier_factory, mock_updater):
    ...
    mock_notifier = notifier_factory(ipv4_ready='false')
...
def test_ipv6_ready_false(notifier_factory, mock_updater):
    ...
    mock_notifier = notifier_factory(ipv6_ready='false')
...
def test_skip_ipv4_attach_ipv4(notifier_factory, mock_updater):
    ...
    mock_notifier = notifier_factory(skip_ipv4='true')
    mock_notifier.attach_ipv4_updater(mock_updater.update_ipv4)
    assert not mock_notifier.want_ipv4()
...
def test_skip_ipv6_attach_ipv6(notifier_factory, mock_updater):
    ...
    mock_notifier = notifier_factory(skip_ipv6='true')
    mock_notifier.attach_ipv6_updater(mock_updater.update_ipv6)
    assert not mock_notifier.want_ipv6()
...
def test_ipv4_ready_false_and_skipped(notifier_factory, mock_updater):
    ...
    mock_notifier = notifier_factory(skip_ipv4='true', ipv4_ready='false')
    mock_notifier.attach_ipv4_updater(mock_updater.update_ipv4)
    mock_notifier.attach_ipv6_updater(mock_updater.update_ipv6)
    mock_notifier.notify_ipv6(ipaddress.IPv6Network('1234::/64'))
    mock_notifier.notify_ipv6(ipaddress.IPv6Network('5678::/64'))
...
def test_ipv6_ready_false_and_skipped(notifier_factory, mock_updater):
    ...
    does not raise error"""
    mock_notifier = notifier_factory(skip_ipv6='true', ipv6_ready='false')
    mock_notifier.attach_ipv4_updater(mock_updater.update_ipv4)
    mock_notifier.attach_ipv6_updater(mock_updater.update_ipv6)
    mock_notifier.notify_ipv4(ipaddress.IPv4Address('10.20.30.40'))
    mock_notifier.notify_ipv4(ipaddress.IPv4Address('50.60.70.80'))
...
def test_skip_both_error():
    ...
    with pytest.raises(ruddr.ConfigError):
        mocks.MockNotifier('mock_notifier', notifier_config)


def test_multiple_updaters(notifier_factory, updater_factory):
    """Test multiple updaters attached to a single notifier"""
    mock_notifier = notifier_factory()
    mock_updater_1 = updater_factory()
    mock_updater_2 = updater_factory()
    mock_notifier.attach_ipv4_updater(mock_updater_1.update_ipv4)
    mock_notifier.attach_ipv6_updater(mock_updater_1.update_ipv6)
    mock_notifier.attach_ipv4_updater(mock_updater_2.update_ipv4)
    mock_notifier.attach_ipv6_updater(mock_updater_2.update_ipv6)
